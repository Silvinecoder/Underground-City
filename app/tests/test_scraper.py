...
def read_html():
      relative_path = './morrisons.html'
      absolute_path = os.path.abspath(relative_path)
      with open(absolute_path) as file:
        return file.read()
      
def supermarkets_morrisons_scrape(html_content):
    soup = BeautifulSoup(html_content, 'html.parser')

    product_elements = soup.select('.fop-item')
    # print('Count of product elements:', len(product_elements))

    for product_element in product_elements:
        image_element = product_element.find('a', class_='fop-ribbon__title')
        product_text = product_element.find('h4', class_='fop-title')
        # product_text.get('title', '')
        print('title', product_text.get('title', ''))

        # if image_element:
        #     product_name = image_element.get('title', '')
        #     print('Product Name:', len(product_name))
        #     product_image_url = image_element.get('href', '')
        #     print('image url', product_name, product_image_url)
...
def scraper_test():
    ...
    scrapedData = supermarkets_morrisons_scrape(read_html())
